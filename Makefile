...
bash-app: ## Run bash inside container app
...
composer: ## Run npm inside container app
	$(info --> Run composer inside the container app)
	@docker-compose exec app composer $(RUN_ARGS)

bash-node: ## Run bash inside container node
	$(info --> Run bash inside the container node)
